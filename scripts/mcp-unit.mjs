...
assert(landing.safety.liveDualWrites === false, "MCP landing reports no live writes");
...
assert(init.safety.writeTools === "none", "MCP initialize reports no write tools");
...
assert(![...toolNames].some((name) => /sync|mint|update|write|execute/i.test(name)), "MCP exposes no write-like tools");
...
assert(manifest.contents[0].text.includes("/Users/ibuswell/Documents/DualVault"), "MCP manifest includes project vault");
assert(manifest.contents[0].text.includes("\"liveDualWrites\": false"), "MCP manifest declares no live writes");
...
assert(writeBoundaryPrompt.messages[0].content.text.includes("no sync"), "MCP write-boundary prompt checks no sync path");
...
assert(evalResult.structuredContent.liveDualWrites === false, "MCP evaluate reports no live writes");
